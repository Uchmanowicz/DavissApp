...
namespace Managers
{
	...
	JobHistory JobManagement::select(const std::string &login_inTimestmap, Status *status) const
	{
		DB::Query query;
		query.query = QString("SELECT * FROM %1 WHERE %2 = '%3';")
						  .arg(JobHistoryTable::TABLE_NAME, JobHistoryTable::LOGIN_TIMESTAMP, login_inTimestmap.c_str())
						  .toStdString();
	...
	std::vector<JobHistory> JobManagement::selectAll(Status *status) const
	{
		DB::Query query;
		query.query = QString("SELECT * FROM %1 WHERE %2 = '%3';")
						  .arg(JobHistoryTable::TABLE_NAME, JobHistoryTable::LOGIN, UserSession::getInstance().getUser().login)
						  .toStdString();
	...
	bool JobManagement::insert(const JobHistory &jobHistory, Status *status)
	{
		DB::Query query;
		query.query = QString("INSERT INTO %1(%2, %3, %4, %5) VALUES('%6', '%7', '%8', '%9');")
						  ...
						  .arg(jobHistory.login_inTimestamp, QString::number(jobHistory.outTimestamp), QString::number(jobHistory.duration), UserSession::getInstance().getUser().login)
						  .toStdString();
	...
	bool JobManagement::remove(const JobHistory &jobHistory, Status *status)
	{
		DB::Query query;
		query.query = QString("DELETE FROM %1 WHERE %2 = '%3';")
						  .arg(JobHistoryTable::TABLE_NAME, JobHistoryTable::LOGIN_TIMESTAMP, jobHistory.login_inTimestamp)
						  .toStdString();
	...
	bool JobManagement::removeAll(Status *status)
	{
		DB::Query query;
		query.query = QString("DELETE FROM %1 WHERE %2 = '%3';")
						  .arg(JobHistoryTable::TABLE_NAME, JobHistoryTable::LOGIN, UserSession::getInstance().getUser().login)
	...
	bool JobManagement::update(const JobHistory &jobHistory, Status *status)
	{
		DB::Query query;
		query.query = QString("UPDATE %1 SET %2='%3', %4='%5', %6='%7', %8='%9' WHERE %10 = '%11';")
						  ...
						  .arg(JobHistoryTable::DURATION, QString::number(jobHistory.duration), JobHistoryTable::LOGIN, UserSession::getInstance().getUser().login)
						  .toStdString();
	...
	JobHistory JobManagement::selectWeb(const std::string &login_inTimestmap, Status *status) const
	{
		DB::Query query;
		query.query = QString("/%2/%3").arg(JobHistoryTable::TABLE_NAME, login_inTimestmap.c_str()).toStdString();
		query.type = DB::Query::Type::SELECT;
	...
	std::vector<JobHistory> JobManagement::selectAllWeb(Status *status) const
	{
		DB::Query query;
	...
	bool JobManagement::insertWeb(const JobHistory &jobHistory, Status *status)
	{
		DB::Query query;
		query.query = QString("/%1").arg(JobHistoryTable::TABLE_NAME).toStdString();
		query.body = toJsonWeb(jobHistory);
		query.type = DB::Query::Type::INSERT;

		return webManager->execQuery(query, status);
	}

	bool JobManagement::removeWeb(const JobHistory &jobHistory, Status *status)
	{
		DB::Query query;
		query.query = QString("/%1/%2").arg(JobHistoryTable::TABLE_NAME, jobHistory.login_inTimestamp).toStdString();
		query.type = DB::Query::Type::REMOVE;

		return webManager->execQuery(query, status);
	}

	bool JobManagement::removeAllWeb(Status *status)
	{
		DB::Query query;
		query.query = QString("/%1?filter=%2,eq,%3").arg(JobHistoryTable::TABLE_NAME, JobHistoryTable::LOGIN, UserSession::getInstance().getUser().login).toStdString();
		query.type = DB::Query::Type::REMOVE;
	...
	bool JobManagement::updateWeb(const JobHistory &jobHistory, Status *status)
	{
		DB::Query query;
		query.query = QString("/%2/%3").arg(JobHistoryTable::TABLE_NAME, jobHistory.login_inTimestamp).toStdString();
		query.body = toJsonWeb(jobHistory);
		query.type = DB::Query::Type::UPDATE;
