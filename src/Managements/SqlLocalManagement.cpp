...
namespace DB
{
	...
	QSqlRecord SqlLocalManagement::getRecord(const DB::Query &query, DB::Status *status) const
	{
	...
	std::vector<QSqlRecord> SqlLocalManagement::getRecords(const DB::Query &query, DB::Status *status) const
	{
		QString sql(query.query.c_str());
		QSqlQuery sqlQuery(m_database);
	...
	bool SqlLocalManagement::execQuery(const DB::Query &query, DB::Status *status) const
	{
		QString sql(query.query.c_str());
		QSqlQuery sqlQuery(m_database);
	...
	void SqlLocalManagement::fetchError(const QSqlError &sqlErr, DB::Status *status) const
	{
		switch(sqlErr.type()) {
