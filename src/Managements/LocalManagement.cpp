...
QSqlRecord LocalManagement::getRecord(const Query &query, DBStatus::StatusType *status) const
{
...
std::vector<QSqlRecord> LocalManagement::getRecords(const Query &query, DBStatus::StatusType *status) const
{
	QString sql(query.query.c_str());
	QSqlQuery sqlQuery(m_database);
...
bool LocalManagement::execQuery(const Query &query, DBStatus::StatusType *status) const
{
	QString sql(query.query.c_str());
	QSqlQuery sqlQuery(m_database);
...
void LocalManagement::fetchError(const QSqlError &sqlErr, DBStatus::StatusType *status) const
{
	switch(sqlErr.type()) {
