...
namespace Managers
{
	UserManagement::UserManagement(const QSqlDatabase &database, const std::shared_ptr<DB::IWebDbExecutor> &databaseWeb) //, const std::shared_ptr<Synchronizer> &synchronizer_)
	...
	User UserManagement::select(const std::string &login, DB::Status *status) const
	{
		DB::Query query;
	...
	std::vector<User> UserManagement::selectAll(DB::Status *status) const
	{
		DB::Query query;
	...
	bool UserManagement::insert(const User &user, DB::Status *status)
	{
		DB::Query query;
	...
	bool UserManagement::remove(const User &user, DB::Status *status)
	{
		if(user.login == "Guest") {
			return true;
		}
	...
	bool UserManagement::removeAll(DB::Status *status)
	{
		DB::Query query;
		query.query = QString("DELETE FROM %1 WHERE %2 = '%3';")
	...
	bool UserManagement::update(const User &user, DB::Status *status)
	{
		DB::Query query;
	...
	User UserManagement::selectWeb(const std::string &login, DB::Status *status) const
	{
		DB::Query query;
	...
	std::vector<User> UserManagement::selectAllWeb(DB::Status *status) const
	{
		DB::Query query;
	...
	bool UserManagement::insertWeb(const User &user, DB::Status *status)
	{
		DB::Query query;
	...
	bool UserManagement::removeWeb(const User &user, DB::Status *status)
	{
		DB::Query query;
	...
	bool UserManagement::removeAllWeb(DB::Status *status)
	{
		return 0;
	}

	bool UserManagement::updateWeb(const User &user, DB::Status *status)
	{
		auto qrTxt = QString("/%1/%2").arg(UserTable::TABLE_NAME, user.login).toStdString();
		DB::Query query(qrTxt, fetchUserToJsonWeb(user), DB::Query::UPDATE);
