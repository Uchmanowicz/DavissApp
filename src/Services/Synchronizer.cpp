...
namespace Services
{
	...
	void Synchronizer::onAppChanged(const App::Settings &app)
	{
		currentApp = app;
		if(currentApp.sync && currentUser.isLogged() && stopFlag) {
			startSynchronizing();
	...
	void Synchronizer::onLocalUpdates(const JobHistory &jobHistory)
	{
		...
		updateLocalSyncTimestamp(&Sync::Entities::jobSync);
	}
	...
	void Synchronizer::onLocalUpdates(const User &user)
	{
		...
		updateLocalSyncTimestamp(&Sync::Entities::userSync);
	}

	void Synchronizer::reset()
	{
		std::lock_guard<std::mutex> lock(mutex_syncData);
		localSyncData = Sync::Entities();
		webSyncData = Sync::Entities();
	}
	...
	{
		webDatabase = std::make_shared<DB::WebMySqlDbExecutor>(domain.c_str());
		database = std::make_shared<DB::SqliteDbExecutor>(dbPath.c_str());
	...
	void Synchronizer::syncUser()
	{
		...
		if(successUpdate) {
			qDebug() << "Sync USER!";
			mergeSyncTimestamp(&Sync::Entities::userSync);
	...
	void Synchronizer::syncJob()
	{
		...
		if(successUpdate) {
			qDebug() << "Sync JOB!";
			mergeSyncTimestamp(&Sync::Entities::jobSync);
			publishSync<Publisher<IJobSync>>();
		}
	}

	void Synchronizer::updateLocalSyncTimestamp(long long Sync::Entities::*syncModule)
	{
		DB::Status dbStatus;
	...
	void Synchronizer::mergeSyncTimestamp(long long Sync::Entities::*syncModule)
	{
		DB::Status dbStatus;
